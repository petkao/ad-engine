...
async function semanticSearch(queryText, limit = 12, category = null, forReranking = false) {
  ...
  let sql = `
    SELECT
      ...
      s.id as seller_id, s.name as seller_name, s.industry, s.location as seller_location, s.is_verified as seller_verified, s.geo_verified as seller_geo_verified,
      ae.transcript,
  ...
  try {
    const { rows } = await pool.query(`
      SELECT a.id, a.headline, a.body_copy, a.format, a.media_url,
             ...
             p.title as product_title, p.price, p.category, p.currency, p.product_url, p.image_url as product_image_url,
             s.id as seller_id, s.name as seller_name, s.location as seller_location, s.is_verified as seller_verified, s.geo_verified as seller_geo_verified
      FROM ads a
      JOIN products p ON a.product_id = p.id
      JOIN sellers s ON p.seller_id = s.id
      WHERE a.status = 'active'
